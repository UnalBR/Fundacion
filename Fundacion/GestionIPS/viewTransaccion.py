...
class TransaccionView(View):
    ...
    def get (self,request,Transac=""):
        if len(Transac)>0:
            ...
            if len(transaccionNu)>0:
                datos={'Transaccion':transaccionNu}
            else:
                datos={'mensaje4':"No se encontro el Oficio"}
        else:
            transacciones=list(Transaccion.objects.values())
            if len(transacciones)>0: 
                datos={'Transacciones':transacciones}
            else:
                datos={'mensaje4.1':"No se emcontraton Oficios"}
    ...
    def post(self,request):
        ...
        try:
            ...
            transaccionTe.save()
            datos={'mensaje4.2':'Libro resgistrado esxitosamente'}
    ...
    def put(self,request,Transac):
        ...
        if len(transaccionMo)>0:
            ...
            tra.save()
            mensaje={"mensaje4.3":"Libro Actualizado exitosamente."}
        else:
            mensaje={"mensaje4.4":"No se encontro el Libro."}
    ...
    def delete(self,request,Transac):
        ...
        if len(transaccionDe)>0:
            ...
            mensaje={"mensaje4.5":"Libro Eliminado exitosamente."}
        else:
            mensaje={"mensaje4.6":"No se encontro el Libro."}
